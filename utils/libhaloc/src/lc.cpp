...
/** \brief LoopClosure class constructor.
  */
haloc::LoopClosure::LoopClosure(bool checkLC) : m_infer(checkLC)
{
...
void haloc::LoopClosure::init()
{
  ...
  params_.work_dir += "/features/";
  params_.image_dir += "/images/";
  ROS_INFO_STREAM("workdir = "<< params_.work_dir);
  ROS_INFO_STREAM("imgdir =  "<< params_.image_dir);
  if (m_infer)
  {
    // Create the directory to store the keypoints and descriptors
    std::string feaInfer = params_.work_dir + "infer";

    if (fs::is_directory(feaInfer))
      fs::remove_all(feaInfer);
    fs::path dir(feaInfer);
    if (!fs::create_directory(dir))
    {
      ROS_ERROR("[Haloc:] ERROR -> Impossible to create the execution directory.");
    }

    //clear infer image directory
    if (params_.save_images)
    {
     // Create the directory to store the keypoints and descriptors
        std::string imgInfer = params_.image_dir +"infer";
      if (fs::is_directory(imgInfer))
        fs::remove_all(imgInfer);
      fs::path dir(imgInfer);
      if (!fs::create_directory(dir))
      {
        ROS_ERROR("[Haloc:] ERROR -> Impossible to create the image directory.");
      }

      // clear loops directory
      std::string loopsDir = params_.image_dir + "loops";
      if (fs::is_directory(loopsDir))
        fs::remove_all(loopsDir);
      fs::path dirLoop(loopsDir);
      if (!fs::create_directory(dirLoop))
      {
        ROS_ERROR("[Haloc:] ERROR -> Impossible to create the execution directory.");    
      }
    }
  ...
  { 
    ...
    std::string feaLearning = params_.work_dir + "learning";
    ROS_INFO_STREAM("feaLearning =  " << feaLearning);
    if (fs::is_directory(feaLearning))
      fs::remove_all(feaLearning);
    fs::path dir(feaLearning);
    if (!fs::create_directory(dir))
    {
      ROS_ERROR("[Haloc:] ERROR -> Impossible to create the execution directory.");
    }
    ...
    {
     // Create the directory to store the keypoints and descriptors
        std::string imgLearning = params_.image_dir +"learning";
      if (fs::is_directory(imgLearning))
        fs::remove_all(imgLearning);
      fs::path dir(imgLearning);
      if (!fs::create_directory(dir))
      {
        ROS_ERROR("[Haloc:] ERROR -> Impossible to create the image directory.");
      }
...
int haloc::LoopClosure::setNode(Mat img_l, Mat img_r)
{
  if (m_infer)
  {  // infer
	  int query_id = hash_table_.size();
	  if (!query_.setStereo(query_id, img_l, img_r))
	    return -1;

	  if (params_.save_images)
	  {

        std::string path = params_.image_dir + "infer/" + lexical_cast<string>(query_id) + "_l.png";
        imwrite(path, img_l);
      }
      // Initialize hash
      if (!hash_.isInitialized())
      ...
      // Save kp and descriptors
      FileStorage fs(params_.work_dir+"infer/"+lexical_cast<string>(query_id)+".yml", FileStorage::WRITE);
      write(fs, "id",     query_.getId());
      write(fs, "kp",     query_.getKp());
      write(fs, "desc",   query_.getDesc());
      ...
      // Save hash to table 
      cur_hash = make_pair(query_.getId(), hash_.getHash(query_.getDesc()));
      //hash_table_.push_back(cur_hash);
      return query_id;
  }
  else //learning
  {
      ...
      {
  		char filename[80];
  		sprintf(filename, "%05d_l.png", img_id_);
  		string path = params_.image_dir + "learning/"+ std::string(filename);
    	//string path = params_.image_dir + "learning/"+ lexical_cast<string>(img_id_) + "_l.png";
        imwrite(path, img_l);
        //path = params_.image_dir + "learning/" + lexical_cast<string>(img_id_) + "_r.png";
        //imwrite(path, img_r);

        //======== Add for debug =================
        // prepare visualization windows
        // get image
/*        cv::Mat im = cv::imread(path.c_str(), 0); // grey scale
      ...
      // Save hash to table 
      cur_hash = make_pair(query_.getId(), hash_.getHash(query_.getDesc()));
      // cur_hash = make_pair(img_id_, hash_.getHash(query_.getDesc()));                   // @bob mark
      ROS_INFO("query_: %d   img_id_: %d  cur_hash: %d", query_.getId(), img_id_, cur_hash.first);
      hash_table_.push_back(cur_hash);


      // Save kp and descriptors
      FileStorage fs(params_.work_dir+"learning/"+lexical_cast<string>(img_id_)+".yml", FileStorage::WRITE);
...
{
  getCandidates(img_id_, candidates);
}
                                        ...
                                        tf::Transform& trans)
{
  ...
  if (hash_matching.size() == 0) 
  {
  //Remove the last hash value in hash table 
  ...
  for (uint i=0; i<hash_matching.size(); i++)
  {
    ...
    Image candidate = getImage(params_.work_dir+"learning/"+candidate_id+".yml");
  ...
  if (valid)
  {
    ...
    lc_found_.push_back(make_pair(lc_img_id, (img_id_) ));

    // Show loop closure image
    /*  string path_query = params_.image_dir + "infer/" + lexical_cast<string>(lc_img_id) + "_l.png";
      string path_candidate = params_.image_dir + "infer/" + lexical_cast<string>(img_id_ - 1) + "_l.png";
      Mat img_query = imread(path_query, CV_LOAD_IMAGE_COLOR);
      Mat img_candidate = imread(path_candidate, CV_LOAD_IMAGE_COLOR);
      std::string lcpPath = "/home/bob/source_code/workspace/haloc_output/work_dir/lc/" + lexical_cast<string>(img_id_ - 1) + "_p.png";
      std::string lcvPath = "/home/bob/source_code/workspace/haloc_output/work_dir/lc/" + lexical_cast<string>(img_id_ - 1) + "_c.png";
      imwrite(lcpPath, img_candidate);
      imwrite(lcvPath, img_query); */
...
{
  // Image a
  string file_a = params_.work_dir+"learning/"+lexical_cast<string>(img_id_a)+".yml";
  Image img_a = getImage(file_a);

  // Image b
  string file_b = params_.work_dir+"infer/"+lexical_cast<string>(img_id_b)+".yml";
                                 ...
                                 tf::Transform& trans)
{
  ...
  cout << "number of matched feature after CROSSCHECK: " << matches << endl;
  ...
  else // Stereo
  {
    ...
    cout << "Number of inliers after solvePnP: " << inliers << endl;
    if (inliers < params_.min_inliers)
      return false;
    ...
    if (params_.save_images)
    {
      ...
      Mat img_inliers;
      string path_query = params_.image_dir + "infer/" + lexical_cast<string>(query.getId()) + "_l.png";
      char filename[80];
      sprintf(filename, "%05d_l.png", candidate.getId());
      string path_candidate = params_.image_dir + "learning/" + filename;
      cout << "query_img = " << path_query <<endl;
      cout << "candidate_img = " << path_candidate <<endl; 
      //ROS_INFO("query_img = %s", path_query);
      //ROS_INFO("candidate_img = %s", path_candidate); + 
      string output = params_.image_dir + "loops/" + "learning_" + lexical_cast<string>(candidate.getId()) + "-infer_" + lexical_cast<string>(query.getId()) + ".png";
      Mat img_query = imread(path_query, CV_LOAD_IMAGE_COLOR);
      Mat img_candidate = imread(path_candidate, CV_LOAD_IMAGE_COLOR);
      drawMatches(img_candidate, candidate_kp, img_query, query_kp, final_inliers, img_inliers);
      cout << "Save the matching 1" << endl;
      imwrite(output, img_inliers);
      cout << "Save the matching 2" << endl;
    }

    trans = haloc::Utils::buildTransformation(rvec, tvec);
    cout << "Trans is GOOD!" << endl;
...
/** \brief Compute 3d transform .
  * \param candidateID the candidate id 
  * \param trans       return transformation
  * \author bob
  */
void haloc::LoopClosure::Compute3D(int candidateID, tf::Transform& trans)
{
    trans.setIdentity();
    int matches = 0;
    int max_matches = 0;
    ...
    string candidate_id = lexical_cast<string>(candidateID);
    haloc::Image candidate = getImage(params_.work_dir+"learning/"+candidate_id+".yml");
    valid = compute(query_, candidate, matches, inliers, trans);
    if (valid)
