...
public:
  ...
  struct Params
  {
    ...
    static constexpr const double       DEFAULT_DESC_THRESH_RATIO = 0.9; //0.8
    ...
    static const int                    DEFAULT_MIN_MATCHES = 10;   //20
    static const int                    DEFAULT_MIN_INLIERS = 5;   //12 /5   //it might result incorrect transformation solving.
    static constexpr const double       DEFAULT_MAX_REPROJ_ERR = 2.0;
  ...
  // Save hash table into txt file
  std::pair<int, std::vector<float> >  getCurHash(){return cur_hash;}

  void setHashData(vector< pair<int, vector<float> > >& _hashTable)
  { 
    hash_table_ =  _hashTable;
  }

  void Compute3D(int , tf::Transform& );
...
private:
  ...
  pair<int, vector<float> >  cur_hash;
