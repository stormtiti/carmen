...
public:
  ...
  void allocate(int nentries, int nkeys = 0);
    ...
    // ============== Add by Samuel ===================
  void setLearningDatabase(const int image_id,
...
/*  if((int)entry_id <= m_params.dislocal) @ Samuel
  {
...
//    int max_id = (int)entry_id - m_params.dislocal;
    ...
    QueryResults qret;
    m_database->query(bowvec, qret, m_params.max_db_results, m_database->size());

    // update database
    // m_database->add(bowvec, featvec); // returns entry_id      @ bob
...
/*  if(m_image_keys.size() == entry_id)  @ Samuel
  {
  ...
  // store this bowvec if we are going to use it in next iterations
...
// ============== Add by Samuel ===================
template<class TDescriptor, class F>
void TemplatedLoopDetector<TDescriptor, F>::setLearningDatabase(
		const int image_id,
		const std::vector<cv::KeyPoint> &keys,
		const std::vector<TDescriptor> &descriptors)
{
  ...
  // only add the entry to the database and finish
  m_database->add(bowvec, featvec);
  m_image_keys[image_id] = keys;
  m_image_descriptors[image_id] = descriptors;
}
  // ================================================
